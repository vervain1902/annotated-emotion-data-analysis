...
outputFolder = 'E:\2023国自科多模态\VA研究\05-even-A';
...
% 获取文件夹下所有 Excel 文件
files = dir(fullfile(inputFolder, '*even*.xlsx')); % 只匹配名称含有'even'的文件
...
% 生成新的 Excel 文件名
outputFile = fullfile(outputFolder, 'A-frequency.xlsx');

% 保存统计结果到新的 Excel 文件中
writetable(finalResults, outputFile, 'WriteVariableNames', false);

disp('频数统计已完成并保存至 "A-frequency.xlsx" 文件中。');
